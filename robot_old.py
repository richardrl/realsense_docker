...
class Robot(object):
    def __init__(self, is_sim, obj_mesh_dir, num_obj, workspace_limits,
        ...
        # If in real-settings...
            ...
            # flush first state, the version message
            # self.tcp_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            # self.tcp_socket.connect((self.tcp_host_ip, self.tcp_port))
            # tcp_first_message = self.tcp_socket.recv(2048)
            tcp_first_message = self.get_state()

            self.parse_version_message(tcp_first_message)
    ...
    def parse_version_message(self, data_bytes):
        data_length = struct.unpack("!i", data_bytes[0:4])[0]
        robot_message_type = data_bytes[4]
        timestamp = data_bytes[5:13]
        message_src = data_bytes[14]
        robotMessageType = data_bytes[15]

        projectNameSize = data_bytes[16]

        projectName = struct.unpack(f"!{projectNameSize}c", data_bytes[16:16 + projectNameSize])

        majorVersion = data_bytes[16+projectNameSize:17+projectNameSize]
        miniorVersion = data_bytes[17+projectNameSize:18+projectNameSize]
